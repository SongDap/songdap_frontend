    ...
    async (targetPage?: number) => {
      ...
      try {
        ...
        if (pageData.content.length === 0 && page > 1) {
          ...
          const prevPageData = await getAlbums(sortApi, prevPage - 1, itemsPerPage);
                    ...
                    <AlbumCard
                      ...
                      onDelete={async (id) => {
                        try {
                          ...
                          console.log("[Album List] 앨범 삭제 시작:", {
                            id,
                            currentPageNum,
                            currentSortApi,
                          });
                          ...
                          if (pageData.content.length === 0 && currentPageNum > 1) {
                            ...
                            const prevBase = prevPageData.content ?? [];
                            const prevDetails = await Promise.allSettled(
                              prevBase.map((a) => getAlbum(a.uuid))
                            );
                            const prevMap = new Map<string, any>();
                            prevDetails.forEach((r) => {
                              if (r.status === "fulfilled" && r.value?.uuid)
                                prevMap.set(r.value.uuid, r.value);
                            });
                            setAlbums(
                              prevBase.map((a) => {
